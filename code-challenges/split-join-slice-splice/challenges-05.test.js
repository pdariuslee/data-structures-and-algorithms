...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach( people => {
    ...
    $htmlTemplate.attr('id', '');
    $htmlTemplate.find('h2').text(people.name);
    $htmlTemplate.find('h3').text(people.height);
    $htmlTemplate.find('p').text(people.eye_color);
...
const listFoods = (recipe) => {
  ...
  //use slice and indexOf

  gruffaloCrumble.ingredients.forEach(ingredients => {
    ingredients = ingredients.slice(ingredients.indexOf(' ')+1,ingredients.length);
