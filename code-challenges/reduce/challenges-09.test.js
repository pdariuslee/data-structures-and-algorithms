...
const mapCurrentEvents = () => {
  ...
  return currentEvents.news.map(valuesFromObj => new Event(valuesFromObj));
};
...
const countNumberOfElements = (arr) => {
  ...
  // let counterLength = 1;
  let answer = arr.reduce( (accumulator, value, idx) => {
...
const returnNames = (arr) => {
  ...
  // let namesArr = [];
  const namesArr = arr.reduce( (acc, val, idx, arr) => {

    acc.push(val.name);
    return acc;
  }, []);

  // console.log('ay ' + namesArr);
  return namesArr;
};
...
const reversedString = (str) => {
  ...
  let reversedArr = revStr.reduce( (newString, currentLetter) => {
