...
const toTitleCase = (arr) => {
  ...
  let ans2 = arr.map(word => {

    let altWord1 = word.slice(0,1);
    altWord1 = altWord1.toUpperCase(0,1);
    let altWord2 = word.slice(1, word.length);
    let combWord = altWord1 + altWord2;
...
let biggerThanLuke = (arr) => 
  ...
  // let objLuke = arr.reduce( (acc, val) => {
  ...
  //   if(parseInt(el.mass) > parseInt(objLuke.mass)){
  //     names.push(el.name);
  //   }
  // });

  // let final = `${names[0]} - ${names[1]}`;
  // console.log(final);
  // return final;
